...
def main():
    ...
    qid=qid.split(".")[0]
    if os.path.exists(base_dir+'/error/'+str(uid)) is False:
        os.mkdir(base_dir+'/error/'+str(uid))
    src_userfile=user_dir+"/"+uid+"/"+qid+"/"+filename
    exec_file=user_dir+"/"+uid+"/"+qid+"/"+filename.split(".")[0]
    error_file=error_dir+"/"+uid+"/"+filename.split(".")[0]+'.txt'		#need to check existence before using
    res= []
